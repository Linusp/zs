...
from .consts import (
    DAILY_DIGEST_SCENARIO_TEMPLATE,
    EFB_SCENARIO_TEMPLATE,
    KZ_SCENARIO_TEMPLATE,
)
...
def generate_kz_scenario(name, wxid, topic_id, rsshub_base_url="https://rsshub.app"):
    ...
    for agent in result["agents"]:
        ...
        if agent["type"] == "Agents::RssAgent":
            agent["options"]["url"] = [f"{rsshub_base_url}/kzfeed/topic/{topic_id}"]
        ...
        if agent["type"] == "Agents::RssAgent":
            ...
            if name:
                agent["options"]["instructions"]["title"] = (
                    name + agent["options"]["instructions"]["title"]
                )
